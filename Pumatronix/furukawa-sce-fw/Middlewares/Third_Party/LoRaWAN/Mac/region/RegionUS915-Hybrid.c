...
static uint8_t CountNbOfEnabledChannels (uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels,
										 Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx)
{
...
PhyParam_t RegionUS915HybridGetPhyParam (GetPhyParams_t* getPhy)
{
	...
	switch (getPhy->Attribute) {
		...
		{
			phyParam.Value = ( US915_HYBRID_ACKTIMEOUT
							+ randr (-US915_HYBRID_ACK_TIMEOUT_RND, US915_HYBRID_ACK_TIMEOUT_RND));
			break;
		}
...
bool RegionUS915HybridVerify (VerifyParams_t* verify, PhyAttribute_t phyAttribute)
{
	switch (phyAttribute) {
		case PHY_TX_DR:
		{
			return RegionCommonValueInRange (verify->DatarateParams.Datarate, US915_HYBRID_TX_MIN_DATARATE,
		...
		case PHY_RX_DR:
		{
			return RegionCommonValueInRange (verify->DatarateParams.Datarate, US915_HYBRID_RX_MIN_DATARATE,
...
void RegionUS915HybridComputeRxWindowParameters (int8_t datarate, uint8_t minRxSymbols, uint32_t rxError,
												 RxConfigParams_t *rxConfigParams)
{
	...
	tSymbol = RegionCommonComputeSymbolTimeLoRa (DataratesUS915_HYBRID[rxConfigParams->Datarate],
												 BandwidthsUS915_HYBRID[rxConfigParams->Datarate]);

	radioWakeUpTime = Radio.GetRadioWakeUpTime ();
	RegionCommonComputeRxWindowParameters (tSymbol, minRxSymbols, rxError, radioWakeUpTime,
...
bool RegionUS915HybridRxConfig (RxConfigParams_t* rxConfig, int8_t* datarate)
{
	...
	// Radio configuration
	Radio.SetRxConfig (MODEM_LORA, rxConfig->Bandwidth, phyDr, 1, 0, 8, rxConfig->WindowTimeout, false, 0, false, 0, 0,
	true,
...
bool RegionUS915HybridTxConfig (TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir)
{
	...
	int8_t txPowerLimited = LimitTxPower (txConfig->TxPower, Bands[Channels[txConfig->Channel].Band].TxMaxPower,
...
uint8_t RegionUS915HybridLinkAdrReq (LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut,
									 uint8_t* nbBytesParsed)
{
	...
	status = RegionCommonLinkAdrReqVerifyParams (&linkAdrVerifyParams, &linkAdrParams.Datarate, &linkAdrParams.TxPower,
												 &linkAdrParams.NbRep);
...
uint8_t RegionUS915HybridRxParamSetupReq (RxParamSetupReqParams_t* rxParamSetupReq)
{
	...
	if ((Radio.CheckRfFrequency (freq) == false) || (freq < US915_HYBRID_FIRST_RX1_CHANNEL)
					|| (freq > US915_HYBRID_LAST_RX1_CHANNEL)
					|| (((freq - (uint32_t) US915_HYBRID_FIRST_RX1_CHANNEL)
									% (uint32_t) US915_HYBRID_STEPWIDTH_RX1_CHANNEL) != 0))
	{
		status &= 0xFE; // Channel frequency KO
	}

	// Verify datarate
	if (RegionCommonValueInRange (rxParamSetupReq->Datarate, US915_HYBRID_RX_MIN_DATARATE,
	US915_HYBRID_RX_MAX_DATARATE) == false)
	{
	...
	if (RegionCommonValueInRange (rxParamSetupReq->DrOffset, US915_HYBRID_MIN_RX1_DR_OFFSET,
	US915_HYBRID_MAX_RX1_DR_OFFSET) == false)
	{
								   ...
								   TimerTime_t* aggregatedTimeOff)
{
	...
	if (nextChanParams->AggrTimeOff <= TimerGetElapsedTime (nextChanParams->LastAggrTx))
	{
		...
		*aggregatedTimeOff = 0;
		...
		nbEnabledChannels = CountNbOfEnabledChannels (nextChanParams->Datarate, ChannelsMaskRemaining, Channels, Bands,
...
{
	int8_t txPowerLimited = LimitTxPower (continuousWave->TxPower,
