 ...
 */
static uint8_t LoRaMacNwkSKey[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
									0x00, 0x00 };

/*!
 * AES encryption/decryption cipher application session key
 */
static uint8_t LoRaMacAppSKey[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
									0x00, 0x00 };
...
static void OnRadioRxDone (uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr)
{
	...
	switch (macHdr.Bits.MType) {
		case FRAME_TYPE_JOIN_ACCEPT:
			...
			if (micRx == mic)
			{
				LoRaMacJoinComputeSKeys (LoRaMacAppKey, LoRaMacRxPayload + 1, LoRaMacDevNonce, LoRaMacNwkSKey,
										 LoRaMacAppSKey);
		...
		case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
		{
			...
			else
			{
				...
				LoRaMacComputeMic (payload, size - LORAMAC_MFR_LEN, nwkSKey, address, DOWN_LINK, downLinkCounterTmp,
								   &mic);
				if (micRx == mic)
				{
					isMicOk = true;
		...
		case FRAME_TYPE_DATA_UNCONFIRMED_UP:
			...
			if ((payload != NULL) && (LoRaMacTxPayloadLen > 0))
			{
				...
				if (framePort == 0)
				{
					...
					MacCommandsBufferIndex = 0;
					LoRaMacPayloadEncrypt ((uint8_t*) payload, LoRaMacTxPayloadLen, LoRaMacNwkSKey, LoRaMacDevAddr,
					UP_LINK,
										   UpLinkCounter, &LoRaMacBuffer[pktHeaderLen]);
				}
				else
				{
					LoRaMacPayloadEncrypt ((uint8_t*) payload, LoRaMacTxPayloadLen, LoRaMacAppSKey, LoRaMacDevAddr,
					UP_LINK,
										   UpLinkCounter, &LoRaMacBuffer[pktHeaderLen]);
				}
			}
	...
	switch (mibSet->Type) {
		...
		case MIB_RX2_CHANNEL:
		{
			...
			if (RegionVerify (LoRaMacRegion, &verify, PHY_RX_DR) == true)
			{
				...
				{
					// Compute Rx2 windows parameters
					RegionComputeRxWindowParameters (LoRaMacRegion, LoRaMacParams.Rx2Channel.Datarate,
	...
	switch (mlmeRequest->Type) {
		case MLME_JOIN:
		{
			...
			if ((mlmeRequest->Req.Join.DevEui == NULL) || (mlmeRequest->Req.Join.AppEui == NULL)
							|| (mlmeRequest->Req.Join.AppKey == NULL) || (mlmeRequest->Req.Join.NbTrials == 0))
			{
				return LORAMAC_STATUS_PARAMETER_INVALID;
			}
